...
def test_s4_transitivity():
    ...
    direct     = [0.6, 0.9, 1.3]
    transitive = direct + [0.5, 0.7]
    assert max(transitive) <= max(direct)
...
def test_paraconsistent_threshold():
    """Extended threshold: P∧¬P is coherent-but-undesignated."""
    C2 = Context(threshold=2.0)
    P  = Pattern(1, 1.0)
    nc = G_and(P, G_neg(P))
    assert nc.val == 0
    assert C2.coherent(nc)
    assert not C2.valid(nc)
