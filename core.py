...
def G_neg(p: Pattern) -> Pattern:
    ...
    Negation gradient.
    G_neg(v, L) = (1 − v, L).
    Flips designation; preserves load (the cost of asserting ¬P equals
    the cost of asserting P). Double negation: G_neg(G_neg(P)) = P.
    Definition 3.1.
    """
...
def G_or(p: Pattern, q: Pattern) -> Pattern:
    ...
    Designated iff at least one input is designated. Propagates along
    the path of least gradient demand (minimum load). Definition 3.3.
    """
