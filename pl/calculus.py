"""
pl/calculus.py — Propagation Logic: calculus regime (V = R)

The same mechanism — loaded patterns, gradient fields, propagation
operator — instantiated over a real carrier V = R instead of {0,1}.

    P = (v_P, L_P)  with  v_P in R
...
class CalcPattern:
    """
    A loaded pattern over the real carrier V = R.
    ...
    def __truediv__(self, other):
        """
        G_div (quotient rule):
        v = v_P / v_Q,  L = (L_P*v_Q - v_P*L_Q) / v_Q^2
        """
        if not isinstance(other, CalcPattern):
            other = CalcPattern(other, 0.0)
        return CalcPattern(
    ...
    def tan(self):
        """G_tan: v = tan(v_P),  L = L_P / cos^2(v_P)"""
        c = math.cos(self.val)
...
def integrate(f, a: float, b: float, threshold: float = 1e-8):
    ...
    threshold (the coherence threshold for this operation).
    ...
    directions — the same structural relationship as not-not-P = P.
...
def newton_reconfigure(f_pattern, x0: float, threshold: float = 1e-10,
    ...
    residual demand, so four steps reduce demand from 10^-1 to below
    10^-12.
    ...
    x0        : initial value
    threshold : coherence threshold (halt when |f(x)| < threshold)
